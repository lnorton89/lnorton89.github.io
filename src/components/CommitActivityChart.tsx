  ...
  return (
    <div className={`h-full ${embedded ? "lg:border-l lg:border-hairline lg:pl-6" : "rounded-lg border border-hairline bg-surface/80 p-5 backdrop-blur-sm"}`}>
      ...
      {!hasCompleteData ? (
        ...
        <p className="flex h-[120px] items-center justify-center text-center font-mono text-xs text-text-faint">
          No commits {attribution} in this window
        </p>
      ) : (
      <div ref={ref} className="relative h-[120px]">
        {width > 0 && (
          <svg
            ...
            height={HEIGHT}
            role="img"
            aria-label={`Weekly commits ${attribution} over the last ${windowWeeks} weeks`}
            tabIndex={0}
        ...
        {active && (
          <div
            className="pointer-events-none absolute -top-1 z-10 -translate-x-1/2 -translate-y-full rounded-md border border-hairline bg-surface-raised px-2.5 py-1.5 text-center font-mono text-[11px] text-text shadow-lg"
            style={{ left: active.x }}
          >
            <div className="text-text-faint">Week of {active.week.weekStart}</div>
            <div className="text-cyan">{formatNumber(active.week.commits ?? 0)} commits</div>
          </div>
        )}
        <span className="sr-only" aria-live="polite">
          {active ? `${formatNumber(active.week.commits ?? 0)} commits in the week of ${active.week.weekStart}` : ""}
        </span>
      </div>
      )}
      {hasCompleteData && (
        <p className="mt-3 font-mono text-xs text-text-muted">
          {formatNumber(totalRecent)} commits {attribution} in last {windowWeeks} weeks
        </p>
      )}
    </div>
  );
